# chromviz: design notes for sequence-matching enhancements
...
chromviz reads an assembler's GFA, classifies segments from graph-intrinsic
...
(`real_data/chromviz_fusarium_blast_commands.sh`) offering three `blastn`
...
A recurring tension worth stating up front: chromviz is currently offline and
...
- **Question it answers:** what is this segment, really? For segments chromviz
...
- **Verified tool/DB:**
  - `blastn` against NCBI `core_nt`. NCBI made `core_nt` the default web
    ...
    task of "which chromosome does this piece come from." For chromviz's use
...
- **Question it answers:** is this tandem array (edge_10) a ribosomal DNA (rDNA)
  ...
  off the backbone.
- **What you extract:** tandem-array and telomere-flagged segments (chromviz
...
- **Verified tool/DB:**
  ...
  - **Telomeric repeats:** tidk (tolkit/telomeric-identifier, "Telomere
    ...
    them in windows across the assembly. This corroborates chromviz's own motif
...
- **Question it answers:** is the circular low/variant-coverage segment the
  ...
  organelle rather than a nuclear chromosome? chromviz already flags edge_11 as
...
- **Verified tool/DB (three real options, different mechanisms):**
  - **oatk** (c-zhou/oatk) with **OatkDB** HMM profile databases. `oatk`/
    ...
    assembly GFA via `-G` (not just reads), which matches chromviz's input.
...
- **Usefulness/effort:** Useful as confirmation, but moderate-to-high effort and
  arguably heavier than the question needs. chromviz already infers "circular +
...
- **Question it answers:** is this segment foreign (a different organism, an
  ...
  relevant to edge_4, chromviz's AT-rich contaminant candidate.
...
- **How it flows back:** a "flagged as possible contaminant (FCS-GX: <taxon>)"
  ...
  call chromviz already makes tentatively about edge_4.
- **Usefulness/effort:** High usefulness (contamination is a real reason a
  ...
  "chromviz emits the FASTA and a suggested FCS command; you run it on a server
...
- **Question it answers:** does the number of chromosomes chromviz proposed match
...
- **Usefulness/effort:** Genuinely useful as a *sanity check*, low-to-moderate
  ...
  - **Counting conventions differ:** cytogenetic haploid number, NCBI's
    ...
    chromviz's "backbones" are three different things. Comparing them naively
  ...
  - **Offline philosophy:** this step is inherently online. Keep it optional and
    behind a flag, and let chromviz run fully without it.

---

## How these fit chromviz's existing design

- All six reuse the established pattern: chromviz classifies from the graph,
...
- **DIAMOND default outfmt columns** differ from the blastn outfmt 6 string
  chromviz hard-codes; the exact column set for a protein path was not verified
...
- **Chromosome-count semantics reconciliation** (cytogenetic haploid n vs NCBI
  `totalNumberOfChromosomes` including organelles vs chromviz backbones) is a
